...
def analyze_single_stock(name, ticker):
    try:
        ...
        # データが20日分未満の銘柄（上場直後など）は弾く
        if df.empty or len(df) < 20: 
            return None
        
        # 🚨修正: API制限を回避するため stock.info は使用しない
        # info = stock.info 
        ...
        return {
            "Ticker": ticker,
            "銘柄名": name,
            "セクター": "情報なし(制限回避)", # info削除によりセクターは一旦ダミー化
            "通貨": currency,
...
# 🚨修正: 動的にUIを切り替える項目は form の外に出す
market_choice = st.sidebar.radio("🌐 対象市場を選択", ["🇯🇵 日本株", "🇺🇸 米国株"])

mode_choice = "主要銘柄"
if market_choice == "🇯🇵 日本株":
    mode_choice = st.sidebar.radio("📊 銘柄モード (日本株)", ["日経225全銘柄", "主要銘柄"])
else:
    st.sidebar.write("📊 銘柄モード: 主要米国株 (TSM等含む)")

# 送信ボタン関連だけを form にまとめる
